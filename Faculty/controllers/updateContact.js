const FacultyDetails=require('../../Admin/models/facultyDetails')
const updateContact=async (req,res)=>{
    // res.send(`The Phn No is changed form ${req.body.old} to ${req.body.new}`)
    try {
        const { facultyId, newContact } = req.body;

        // Find the user by rollNo
        const user = await FacultyDetails.findOne({ facultyId:facultyId });
// console.log(user);
